...
def compute_quality_flags(summary: DatasetSummary, missing_df: pd.DataFrame, df: Optional[pd.DataFrame] = None) -> Dict[str, Any]:
    ...
    flags["has_high_cardinality_categoricals"] = any(
        col.unique > summary.n_rows * 0.8 and not col.is_numeric 
        for col in summary.columns if col.non_null > 0
    )
    
    if df is not None:
        flags["has_suspicious_id_duplicates"] = _check_id_duplicates(df)
        flags["has_many_zero_values"] = _check_zero_values(df, summary)
    else:
...
def _check_id_duplicates(df: pd.DataFrame) -> bool:
    """Проверка дубликатов в ID-подобных колонках"""
...
def _check_zero_values(df: pd.DataFrame, summary: DatasetSummary) -> bool:
    """Проверка большой доли нулей в числовых колонках"""
