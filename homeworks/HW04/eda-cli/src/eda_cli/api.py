...
class QualityFlagsResponse(BaseModel):
    ...
    flags: dict[str, bool] = Field(
        ...,
        description="Булевы флаги качества данных",
    )
...
async def quality_flags_from_csv(file: UploadFile = File(...)) -> QualityFlagsResponse:
    """
    Эндпоинт, который принимает CSV-файл и возвращает только флаги качества данных
    без интегральной оценки и дополнительной информации.
    
    Использует то же EDA-ядро, что и /quality-from-csv, но возвращает
    только булевы флаги из compute_quality_flags.
    """
    ...
    print(f"[quality-flags-from-csv] filename={file.filename!r} flags={flags_bool}")
